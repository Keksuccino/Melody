...
- This workspace is giving you access to multiple "Melody" branches/workspaces, which is a Minecraft Java mod. It uses the MultiLoader layout with shared logic under `common` and loader-specific wrappers under `fabric`, and `forge` or `neoforge` depending on the Minecraft version.
...
- Melody's Gradle projects have an unconventional handling of the `version` variable. In Melody's Gradle projects, this variable is always `1.0.0`, which is intentional and should NEVER be changed.
...
- This workspace gives you access to the actual Melody source workspaces for multiple branches/Minecraft versions.
- The folder name of these should always be something like `melody-0.0.0`, with the `0.0.0` replaced with the Minecraft version the workspace is targeting.
...
- Follow existing packages under `de.keksuccino.melody`, mirroring existing sub-packages to keep cross-loader boundaries clear.
- Name resources with the `melody` prefix (e.g., `melody.mixins.json`, `melody.accesswidener`) so Gradle and the loaders resolve them consistently.
...
- Place shared mixins under `common/src/main/java/de/keksuccino/melody/mixin/mixins/common/<side>` and mirror the existing folder depth when adding new targets.
...
- Suffix every unique field or helper with `_Melody`. Static finals use all caps with `_MELODY`, and injected method names follow the `before/after/on/wrap/cancel_<VanillaMethod>_Melody` pattern. Accessor/invoker methods also end in `_Melody`.
...
- Melody has access to Mixin Extras.
- Prefer using features from Mixin Extras instead of using normal Mixin redirects or overrides.
- When leveraging Mixin Extras (`WrapOperation`, `WrapWithCondition`, etc.), name helpers after the intent (`wrap_..._Melody`, `cancel_..._Melody`) and call the provided `Operation` when returning to vanilla flow.
...
## Minecraft Sources
- You have access to full sources of Minecraft, and even some libraries used by Minecraft and Melody, in `/Volumes/STUFF/CODING/WORKSPACES/Java/Minecraft Mods/.MINECRAFT_SOURCES`.
...
- Sources for some libraries used by Minecraft and Melody are in `/Volumes/STUFF/CODING/WORKSPACES/Java/Minecraft Mods/.MINECRAFT_SOURCES/<minecraft_version>/libraries/`.
